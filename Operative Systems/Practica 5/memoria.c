/*
 * memoria.c
 */

#include "comunes.h"

int CrearMemoriaCompartida(key_t llave, size_t tamano) {
  int shmid;

  shmid = shmget(llave, tamano, IPC_CREAT | PERMISOS);
  if (shmid == -1) {
    perror("[Memoria] Error en shmget");
    return -1;
  }

  return shmid;
}

int ObtenerMemoriaCompartida(key_t llave, size_t tamano) {
  int shmid;

  shmid = shmget(llave, tamano, 0);
  if (shmid == -1) {
    perror("[Memoria] Error al obtener memoria existente");
    return -1;
  }

  return shmid;
}

void *AdjuntarMemoria(int shmid) {
  void *ptr;

  ptr = shmat(shmid, NULL, 0);
  if (ptr == (void *)-1) {
    perror("[Memoria] Error en shmat");
    return NULL;
  }

  return ptr;
}

void DesvincularMemoria(void *ptr) {
  if (shmdt(ptr) == -1) {
    perror("[Memoria] Error en shmdt");
  }
}

void EliminarMemoria(int shmid) {
  if (shmctl(shmid, IPC_RMID, NULL) == -1) {
    perror("[Memoria] Error al eliminar memoria compartida");
  }
}
